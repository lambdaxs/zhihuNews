...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextSetLineWidth(context, cellLineWidth);
    //设置一起点
    CGContextMoveToPoint(context, margin, 0);
    //设置连线另一个点
    CGContextAddLineToPoint(context, rect.size.width - margin, 0);
    //画到view [渲染]
    CGContextStrokePath(context);
}
...
- (void)setStoriesModel:(XSStories *)storiesModel
{
    ...
    _storiesTitle.text = _storiesModel.title;
