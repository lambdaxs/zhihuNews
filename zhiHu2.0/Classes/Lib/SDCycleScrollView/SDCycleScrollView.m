...
#pragma mark - init
+ (instancetype)cycleScrollViewDefaultWithFrame:(CGRect)frame
{
...
#pragma mark - 给模型赋值的时候直接解析模型给视图的图片标题赋值
-(void)setTopStoriesArray:(NSArray *)topStoriesArray
{
    _topStoriesArray = topStoriesArray;
    
    NSMutableArray *imgURLGroup = [NSMutableArray array];
    NSMutableArray *titleGroup = [NSMutableArray array];
    NSMutableArray *idStrGroup = [NSMutableArray array];
    [topStoriesArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        XSTopStories *topModel = obj;
        [imgURLGroup addObject:topModel.image];
        [titleGroup addObject:topModel.title];
        [idStrGroup addObject:topModel.id];
    }];
    
    self.imageURLStringsGroup = imgURLGroup;
    self.titlesGroup = titleGroup;
    self.idStrGroup = idStrGroup;
}
...
{
    if ([self.delegate respondsToSelector:@selector(cycleScrollView:didSelectItemAtIndex:topStoiresIdStr:)]) {
        NSUInteger cellIndex = indexPath.item % self.idStrGroup.count;
        NSNumber *idNum = self.idStrGroup[cellIndex];
        NSString *idStr = idNum.stringValue;
        [self.delegate cycleScrollView:self didSelectItemAtIndex:cellIndex topStoiresIdStr:idStr];
    }
}
